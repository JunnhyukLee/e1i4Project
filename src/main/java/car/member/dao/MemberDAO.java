...
public class MemberDAO implements MemberService {
	...
	public MemberDTO memberDelete(int member_code) {
		...
		try {
			...
			DataSource dataSource = (DataSource) context.lookup("java:comp/env/jdbc");
			connection =dataSource.getConnection();
			String sql = "ALTER TABLE CALCULATE DISABLE CONSTRAINT CALCULATE_MEMBER_PK";
			preparedStatement = connection.prepareStatement(sql);
			preparedStatement.executeUpdate();
			log.info("SQL확인" + sql);
			
			String sql1=" DELETE FROM MEMBER WHERE MEMBER_CODE = ? ";
			preparedStatement = connection.prepareStatement(sql1);
			preparedStatement.setInt(1, member_code);
			preparedStatement.executeUpdate();
			log.info("SQL확인" + sql1);
			
			String sql2=" ALTER TABLE CALCULATE ENABLE CONSTRAINT CALCULATE_MEMBER_PK";
		...
		try {
			...
			if (resultSet != null) {
				if(resultSet.next( )) {
					...
					log.info("회원 코드 확인 - " + resultSet.getInt("member_code"));
					
					memberDTO.setMember_id(resultSet.getString("member_id"));
					log.info("아이디 확인 - " + resultSet.getString("member_id"));
					
					memberDTO.setMember_name(resultSet.getString("member_name"));
					log.info("비밀번호 확인 - " + resultSet.getString("member_name"));
					
					memberDTO.setMember_rating(resultSet.getString("member_rating"));
					log.info("등급 확인 - " + resultSet.getString("member_rating"));
	...
	@Override
	public MemberDTO ratingCheck(MemberDTO memberDTO) {
		Connection connection = null;
		PreparedStatement preparedStatement = null;
		try {
			Context context = new InitialContext();
			DataSource dataSource = (DataSource) context.lookup("java:comp/env/jdbc");
			connection =dataSource.getConnection();
			String sql="update member set member_rating = ?";
			sql +=" where member_code = ?";
			log.info("SQL 확인 - " + sql);
			preparedStatement = connection.prepareStatement(sql);
			preparedStatement.setString(1, memberDTO.getMember_rating());
			preparedStatement.setInt(2, memberDTO.getMember_code());
